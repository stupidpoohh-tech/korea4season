...
지도 상단 컨트롤도 이 두 축을 섞지 않습니다.

- **시즌 필터** (전체 · 피크 · 잡기 좋은 때 · 곧 시작) — 지금 잘 잡히는가
- **규정 필터** (`⚠ 규정 있는 어종만`) — 잡아도 되는가. 0건이면 그리지 않는다
...
| `/week?date=` | 이번 주 뭐 잡으러 갈까 | 서버 | 지도 상단 "이번 주 추천" |
...
측정값 (2026년 12개월 전수, 데스크톱 1360px):

| 항목 | 값 |
|---|---|
| 육지 위 sprite | 0개 |
| 최악 sprite 간격 | 0.068 (지도 가로폭 대비, 10월) |
| 최대 동시 sprite | 22개 (확대 시 30) |
...
- **모바일에서 지도 크기는 세로가 아니라 가로에 걸려 있습니다.**
  `h-[min(100cqh,130cqw)]` 이라 컨트롤을 줄여도 그 공간이 지도가 아니라
  빈칸이 됩니다. 지도를 더 키우려면 base map 왼쪽의 빈 서해(폭의 약 25%)를
  잘라내야 하고, 그것은 `scripts/generate-base-map.mjs` 의 좌표 범위 문제입니다.
...
1. **base map 좌측 여백 정리** — 모바일 지도 크기의 유일한 병목
2. **`seasons.json` 근거 대조** — 지금 화면이 말하는 모든 판단의 뿌리
3. 권역 모드 대표 어종 반복 완화
4. 남은 3종 sprite
