...
| 카테고리 | 지금 | 지도 단위 | 보기 방식 |
|---|---|---|---|
| 🌊 **바다** | 동작 | 어종 × 해역 / 낚시 권역 | 어종별 · 권역별 |
| 🍁 **단풍** | 동작 | 단풍 명소(산) | 명소별 (지역별은 자리만) |
| 🌸 꽃 | 준비 중 | — | — |
| 🐦 철새 | 준비 중 | — | — |

제목 자체가 선택지입니다 — `지금, 바다 ⌄` 를 눌러 단풍으로 옮깁니다.
꽃 · 철새는 목록에 이름만 있습니다. 누르면 화면을 만들지 않고
`꽃 지도는 준비 중이에요.` 라고만 말합니다.

> **인터페이스는 넷을 위해 만들고, 기능은 하나씩 완성합니다.**
> 카테고리 설정은 `src/domain/nature-categories.ts` 한 곳입니다.
> 꽃을 켜려면 그 카테고리의 데이터 소스를 켜고 레이어 하나를 붙이면 됩니다 —
> 상단 · 필터 · 추천 · 타임라인은 이미 카테고리를 받도록 되어 있습니다.
...
8. 상세에서 `넙치만 지도에서 1년 재생` 을 누르면 지도에 그 어종만 남기고
   1년을 돌린다 — **"이 물고기는 언제 어디에 있나"** 를 한 번에 본다.
9. 제목에서 `🍁 단풍` 으로 옮기고 슬라이더를 10월로 민다 →
   **북쪽 산부터 남쪽으로 산이 물든다.** 핀이 찍히는 것이 아니라 산맥이 물든다.

### 단풍 (Phase 2)

단풍은 이 서비스가 낚시 앱이 아니라는 것을 증명하는 자리입니다.

- 지도의 **산 자체가 물듭니다.** 지도 이미지를 계절마다 갈아 끼우지 않고,
  base map 이 그린 산과 같은 자리에 가을색 산을 덧그립니다
  (`FoliageOverlay` · 좌표는 생성기가 `src/domain/mountains.json` 으로 함께 굽습니다).
  산 하나하나에 데이터가 있는 것이 아니라, 명소 12곳의 상태를 거리로 섞어
  각 산의 물든 정도를 정합니다 — 그래서 절정인 산 둘레부터 붉어지고
  그 띠가 남쪽으로 내려갑니다.
- 명소 12곳: 설악산 · 오대산 · 치악산 · 소백산 · 월악산 · 속리산 · 한라산 ·
  계룡산 · 지리산 · 북한산 · 무등산 · 내장산 (+ 은행나무 · 억새)
- 상태 어휘는 바다와 같은 엔진을 씁니다 — `아직 · 시작 중 · 좋음 · 절정 · 끝물 · 끝남`
- 상세는 넷만 답합니다: 지금 상태 · 첫 단풍 · 절정 구간 · 지도에서 보기
...
```
모바일                                    데스크톱 좌측 레일
지금日지도                          ⓘ     지금, 바다  23종 활동 중        ⓘ
지금 바다에 23종                          절정 13 · 좋음 7 · 보통 3
...
모바일에는 전역 헤더가 없으므로(`lg` 이상에서만 그립니다) 상단 첫 줄이
서비스명 자리입니다. 데스크톱에서는 헤더가 이미 이름을 말하므로
레일에서는 상태를 제목으로 씁니다.
...
`어종 하나만` 은 조건이 아니라 대상을 고르는 축이라 시트가 아니라 상세에서
켭니다. 켜져 있으면 상단에 `넙치만 ✕` 칩이 남습니다.
...
sprite 의 크기는 시즌 강도를 뜻합니다. 규정은 그림을 지우지 않고
**반투명 붉은 색조로 덮어** 알립니다 — 그림 모양 그대로 덮으므로
어떤 물고기인지는 그대로 보이고, 훑어보는 중에도 "지금은 안 된다" 가 먼저
읽힙니다. 체장 같은 조건부 규정은 대부분의 어종에 붙어 있어 지도에 그리면
색조가 배경이 되므로 상세에서만 말합니다.
...
- 어종 sprite **22종 전부 일러스트입니다.** 이모지 폴백은 남아 있지 않습니다.
  누끼가 잘못 따져 옆 그림 조각이 붙어 있던 보리멸·삼치도 고쳤습니다
  (알파 연결 성분에서 가장 큰 덩어리만 남깁니다).
...
- 알림 구독은 브라우저에만 저장됩니다. 발송·계정 연동은 Phase 4 —
  버튼은 `알림받는 중` 이라고 말하지만 지금 실제로 오는 알림은 없습니다.
- 자연도감(`/dex`)은 탭에서 감춰 두었습니다. 갈 수 없는 화면에 기록했다고
  말하지 않도록, 도감이 닫혀 있는 동안에는 기록도 안내 토스트도 띄우지
  않습니다 (`nav-items.ts` 의 `DEX_VISIBLE`). 다시 열면 그대로 돌아옵니다.
