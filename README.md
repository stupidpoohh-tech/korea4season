...
지도는 흰 바탕입니다. 바다는 아예 칠하지 않습니다 — SVG 바탕이 비어 있어
페이지의 흰색이 그대로 비칩니다. 그래서 육지와 바다를 가르는 것은 색이 아니라
해안을 둘러싼 옅은 회색 띠 세 겹과, 육지를 칠한 **뒤에** 긋는 해안선입니다.

> 해안선은 반드시 육지 채움 뒤에 그어야 합니다. 모래 띠가 폭 20 으로 경로를
> 덮기 때문에 그 앞에 그으면 그려도 보이지 않습니다.

색은 `scripts/generate-base-map.mjs` 의 `C` 한 곳에서만 정합니다.
이 지도에서 색을 갖는 것은 그 위에 놓이는 생물 sprite 하나뿐이어야 합니다.

계절 톤(`src/lib/season.ts` 의 `wash`)은 흰 지도에서 아주 얕게만 얹습니다.
계절을 실제로 알리는 자리는 타임라인의 색 칩입니다.
...
| React 19 / TypeScript | — | — |
| Tailwind CSS v4 | 토큰 기반 화이트 UI | 지도까지 흰색이므로 구획은 색이 아니라 선이 나눔 |
